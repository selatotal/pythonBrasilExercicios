...
for votosAtleta in votosAtletas:
	if (votosAtleta > 0):
		print '%d\t%d\t%.2f%%' % (contador, votosAtleta, votosAtleta / float(totalVotos) * 100)
		if (votosAtleta > votosAtletas[melhorJogador]):
			melhorJogador = contador - 1
	contador += 1

print 'O melhor jogador foi o numero %d, com %d votos, correspondendo a %.2f do total de votos' % (melhorJogador + 1, votosAtletas[melhorJogador], votosAtletas[melhorJogador] / float(totalVotos) * 100)
